	...
	for {
		...
		case <-ticker.C:
			if err := s.schedule(); err != nil {
				s.logger.Error("调度失败", elog.FieldErr(err))
			}
		case <-s.ctx.Done():
...
func (s *Scheduler) schedule() error {
	// 获取可调度的任务列表
	tasks, err := s.taskSvc.SchedulableTasks(s.ctx, s.config.BatchSize)
	if err != nil {
...
func (s *Scheduler) handleAcquiredTask(task domain.Task) {
	...
	// 启动任务执行
	resultChan := s.taskManager.Run(s.ctx, task)
	...
	for {
		...
		case err := <-resultChan:
			...
			s.logger.Error("任务执行失败",
				...
				elog.String("taskName", task.Name),
				elog.FieldErr(err))
			// todo:是否可重新调度，如果是，就是 resultChan := s.taskManager.Run(s.ctx, task)
			return
		case <-renewTicker.C:
			// 续约
			if err := s.taskAcquirer.Renew(s.ctx, task); err != nil {
				...
				// 续约失败，通知任务管理器
				_ = s.taskManager.RenewFailed(task)
				return
			}
